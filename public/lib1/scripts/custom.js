...
$(document).ready(function(){
	...
	$(window).on('load resize', function() {
		...
		$('.fs-inner-container, .fs-inner-container.map-fixed').css('padding-top', 0);
...
function load_project(event) {
	...
	var reader = new FileReader();
	reader.onload = function(){
		var output = document.getElementById('project_img');
		output.src = reader.result;
	};
	reader.readAsDataURL(event.target.files[0]);
	console.log(event.target.files[0]);
}
function load_homeimage(event) {
	var reader = new FileReader();
	reader.onload = function(){
		var output = document.getElementById('homepage_img');
		output.src = reader.result;
	};
	reader.readAsDataURL(event.target.files[0]);
	console.log(event.target.files[0]);
}
function load_contactimage(event) {
	var reader = new FileReader();
	reader.onload = function(){
		var output = document.getElementById('contactpage_img');
		output.src = reader.result;
	};
	reader.readAsDataURL(event.target.files[0]);
	console.log(event.target.files[0]);
...
function setpackage(package_path, package_price, package_name) {
	...
	if(package_price == 0){
		...
		document.getElementById('monthly-amount').value = package_price;
