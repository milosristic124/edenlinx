...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/adminpage', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function providePrefixTests()
    {
        ...
        $tests[] = array('/{adminpage}', '/foo', '/{adminpage}/foo');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        // mount into adminpage, but before the parent collection has been mounted
        $adminRoutes->mount('/blog', $adminBlogRoutes);

        // now mount the /adminpage routes, above should all still be /blog/adminpage
        $routes->mount('/adminpage', $adminRoutes);
        ...
        $loader
            ...
            ->method('load')
            ->will($this->returnValue($importedCollection));
        // import this from the /adminpage route builder
        $adminRoutes->import('adminpage.yml', '/imported');

        $collection = $routes->build();
        $this->assertEquals('/adminpage/dashboard', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/adminpage/users', $collection->get('admin_users')->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/adminpage/blog/new', $collection->get('admin_blog_new')->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/adminpage/stats/sales', $collection->get('admin_stats_sales')->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
        $this->assertEquals('/adminpage/imported/foo', $collection->get('imported_route')->getPath(), 'Normal RouteCollections are also prefixed properly');
    }
