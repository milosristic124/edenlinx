...
class RouteCollectionTest extends TestCase
{
    ...
    public function testAddPrefix()
    {
        ...
        $collection->addPrefix('/{adminpage}', array('adminpage' => 'adminpage'), array('adminpage' => '\d+'));
        $this->assertEquals('/{adminpage}/foo', $collection->get('foo')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals('/{adminpage}/bar', $collection->get('bar')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals(array('adminpage' => 'adminpage'), $collection->get('foo')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('adminpage' => 'adminpage'), $collection->get('bar')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('adminpage' => '\d+'), $collection->get('foo')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $this->assertEquals(array('adminpage' => '\d+'), $collection->get('bar')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $collection->addPrefix('0');
        $this->assertEquals('/0/{adminpage}/foo', $collection->get('foo')->getPath(), '->addPrefix() ensures a prefix must start with a slash and must not end with a slash');
        $collection->addPrefix('/ /');
        $this->assertSame('/ /0/{adminpage}/foo', $collection->get('foo')->getPath(), '->addPrefix() can handle spaces if desired');
        $this->assertSame('/ /0/{adminpage}/bar', $collection->get('bar')->getPath(), 'the route pattern of an added collection is in synch with the added prefix');
    }
    ...
    public function testAddPrefixOverridesDefaultsAndRequirements()
    {
        ...
        $collection->addPrefix('/adminpage', array(), array('_format' => 'html'));
